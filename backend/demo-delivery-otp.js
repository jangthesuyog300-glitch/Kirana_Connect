 ...
 * ============================================================
 * ============================================================
 *  KIRANA CONNECT — OTP FLOW DEMO (DELIVERY)
 *  Demonstrates: Order -> Accept -> Ready -> Dispatched -> Verify
 * ============================================================
 */
...
async function run() {
  banner('KIRANA CONNECT — OTP & DELIVERY DEMO');
  ...
  step(2, 'Customer: Place DELIVERY order');
  const orderRes = await request('POST', '/orders', {
    store_id: storeId,
    items: [{ item_id: 'item-1', quantity: 1 }],
    delivery_type: 'delivery', // CRITICAL
    payment_method: 'cash',
  }, customerToken);
  orderId = orderRes.body.order.id;
  ok('Order Placed', orderId);
  ok('Delivery Type', 'delivery');
  ...
  if (acceptRes.status === 200) {
    ...
    info('This OTP is now visible to the customer and required by the owner for delivery.');
  } else {
    console.log('Failed to accept:', acceptRes.body);
    process.exit(1);
  }

  // 4. Mark Ready & Dispatched
  step(4, 'Store Owner: Mark as READY and then DISPATCHED');
  await request('PATCH', `/orders/${orderId}/status`, { status: 'preparing' }, ownerToken);
  await request('PATCH', `/orders/${orderId}/status`, { status: 'ready' }, ownerToken);
  const dispatchedRes = await request('PATCH', `/orders/${orderId}/status`, { status: 'dispatched' }, ownerToken);
  ok('Status Updated', dispatchedRes.body.order.status);

  // 5. Verify OTP
  step(5, 'Store Owner: Verify Customer OTP to complete delivery');
  ...
  if (verifyRes.status === 200) {
    ...
    console.log(`\n${c.green}${c.bold}🎉 DELIVERY FLOW COMPLETED SUCCESSFULLY${c.reset}\n`);
  } else {
